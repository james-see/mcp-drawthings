...
export const transformImageSchema = z.object({
    ...
    denoising_strength: z
        ...
        .max(1)
        .optional()
        .describe("Strength of the transformation (0.0-1.0, default: 0.75). Lower values keep more of the original image."),
    steps: z
        .number()
        .int()
        .min(1)
...
export const transformImageDescription = "Transform an existing image using a text prompt (img2img). Either image_path or image_base64 must be provided.";
export async function transformImage(client, params) {
    try {
        // Validate input
